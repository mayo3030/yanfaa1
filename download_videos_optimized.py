"""
تحميل الفيديوهات مع تحديد جودة محددة لتجنب تحميل ملفات كبيرة جداً
"""
import json
import subprocess
import sys
...
for i, video in enumerate(videos, 1):
    ...
    if output_file.exists():
        ...
        if file_size_mb > 50:  # إذا كان أكبر من 50 ميجابايت، نعتبره مكتمل
            print(f"   ⏭️ الملف موجود بالفعل ({file_size_mb:.2f} MB) - تخطي\n")
    ...
    try:
        ...
        # تحميل باستخدام ffmpeg مع تحديد جودة متوسطة لتجنب ملفات كبيرة جداً
        # نستخدم -map 0 لتحميل أفضل جودة واحدة فقط، وليس جميع الجودات
        ffmpeg_cmd = [
            'ffmpeg',
            ...
            '-i', hls_url_decoded,
            # تحديد جودة متوسطة (480p أو 720p) لتقليل الحجم
            '-vf', 'scale=1280:720',  # تحويل إلى 720p
            '-c:v', 'libx264',  # re-encode للفيديو
            '-crf', '23',  # جودة متوسطة (أقل = أفضل جودة وأكبر حجم)
            '-c:a', 'aac',  # re-encode للصوت
            '-b:a', '128k',  # معدل بت للصوت
            '-movflags', '+faststart',  # للتحسين
            '-y',  # Overwrite
            str(output_file)
        ]
        
        print(f"   🔄 تشغيل ffmpeg (قد يستغرق بعض الوقت)...")
        print(f"   💡 يتم تحميل وتشفير الفيديو إلى 720p لتقليل الحجم")
        ...
        process = subprocess.run(
            ...
            text=True,
            encoding='utf-8',
            timeout=900  # 15 دقيقة لكل فيديو
        )
        ...
        errors.append(f"{title}: Timeout (15 دقيقة)")
        print(f"   ⏱️ تجاوز الوقت المحدد (15 دقيقة)\n")
